...
import { Purchase } from './purchase-manage.model';
...
export class PurchaseManageComponent {
    ...
    constructor(
        ...
        public purchaseManageService: PurchaseManageService,
    ...
    loadAll() {
        this.purchaseManageService.query({
     ...
     */
    querySupplier() {
        this.purchaseManageService.querySupplier().subscribe(
     ...
     * 修改
     */
    public update(template: TemplateRef<any>, Info: Purchase) {
        this.currentPurchase = Info;
     ...
     * 修改类型
     */
    public changeType(template: TemplateRef<any>, Info: Purchase) {
        this.currentPurchase = Info;
        this.purchaseManageService.queryType({
            query: this.currentPurchase.id
        }).subscribe(
            (res: ResponseWrapper) => {
    ...
    public onSubmit() {
        ...
        if (this.currentPurchase.id != null) {
            this.purchaseManageService.update(this.currentPurchase).subscribe((response) => this.onOperatorSuccess(response));
        } else {
            console.log(this.currentPurchase);
            this.purchaseManageService.create(this.currentPurchase).subscribe((response) => this.onOperatorSuccess(response));
        }
    }
    ...
    public onSubmit1() {
        ...
        this.purchaseManageService.updatePurchase(this.currentPurchase.id, this.proArry);
     ...
     */
    confirm(): void {
        this.purchaseManageService.delete(this.currentPurchase.id).subscribe(() => {
